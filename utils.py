...
from sklearn.model_selection import RandomizedSearchCV, GridSearchCV, RepeatedStratifiedKFold, train_test_split, cross_val_score
...
def get_empty_values_info(df=None, name='', component='column', thresh=35):
    '''
    Prints the number and percentage of emtpy values of each dataframe column and
    collects the columns having an amount by default of NaNs >35%.
    ...
    Input:
        ...
        thresh - threshhold given in percentage, default 35%
    Output:
        dict - dictionary including all the column items with >35% and their calculated NaN information.
    '''
    dict_over35perc = {}
    dict_less36perc = {}
        ...
        if component == 'column':
            ...
            for col in list_clms:
                ...
                if perc_na_col > thresh:
                    dict_over35perc[col] = [sum_na_col, np.round(perc_na_col, decimals=2)]
                else:    # perc_na_col < 36
                    dict_less36perc[col] = [sum_na_col, np.round(perc_na_col, decimals=2)]
    ...
    return dict_over35perc, dict_less36perc
...
def plot_NaN_distribution(feat_labels, values_set1, values_set2, label_set1, label_set2, figsize=(10, 8.5), xmax=6.5):
    '''
...
def plot_histogram(data, xlabel, dataset_name):
    ...
    sns.distplot(a=data, rug=True, ax=ax)
